...
def cmd_api():
    ...
    print(f"[*] API run on http://{host}:{port}")
...
def cmd_scan(filepath: str, n_rows: int = 10000):
    ...
    try:
        ...
        print(f"[!] Model not loaded ({e}), demo-verse")
    ...
    print(f"  Normals:      {n_norm:,}")
    print(f"  Anomalies:        {n_anom:,}  ({pct:.2f}%)")
    print(f"  Threat:          {threat}")
    print(f"{'='*50}\n")

    from notifier import notify_threat
    notify_threat(threat, f"scan {filepath}: {n_anom} anomalies")
...
def cmd_rootkit():
    ...
    print(f"Completed:        {result.passed}")
    ...
    if result.findings:
        ...
        print("  ✅ No signs of a rootkit were detected")

    from notifier import notify_threat
    notify_threat(result.threat_level, f"Rootkit scan: {len(result.findings)} finds")
...
def cmd_monitor(n: int = 20):
    ...
    print(f"{'PID':>6} {'Name':<25} {'CPU%':>6} {'RAM MB':>8} {'Conn':>5} {'Score':>7} {'Threat'}")
    print("-" * 72)
    for r in results[:n]:
        print(
...
if __name__ == "__main__":
    ...
    if cmd == "gui":
        ...
        if len(args) < 2:
            print("[!] Specify the file path: python main.py scan FILE.csv")
            sys.exit(1)
