...
class RootkitGuard(ctk.CTk):
    ...
    def _page_scan(self):
        frame = ctk.CTkFrame(self.main, fg_color="transparent")
        ctk.CTkLabel(frame, text="CSV Сканирование",
                     font=ctk.CTkFont(size=22, weight="bold")).pack(pady=(10, 5))

        ff = ctk.CTkFrame(frame)
        ff.pack(fill="x", padx=20, pady=8)
        ctk.CTkLabel(ff, text="CSV файл:",
                     font=ctk.CTkFont(size=13)).pack(side="left", padx=10, pady=10)
        self.file_path = ctk.CTkEntry(ff, width=430,
                                       placeholder_text="data/raw/friday_traffic.csv")
        self.file_path.pack(side="left", padx=5)
        ctk.CTkButton(ff, text="Обзор", width=80,
                      command=self._browse_file).pack(side="left", padx=5)

        pf = ctk.CTkFrame(frame)
        pf.pack(fill="x", padx=20, pady=4)
        ctk.CTkLabel(pf, text="Порог:").pack(side="left", padx=10, pady=10)
        self.threshold = ctk.CTkSlider(pf, from_=0.1, to=0.9, number_of_steps=8)
        self.threshold.set(cfg.get("scan", {}).get("threshold", 0.5))
        self.threshold.pack(side="left", padx=5)
        self.thresh_lbl = ctk.CTkLabel(pf, text=f"{self.threshold.get():.1f}")
        self.thresh_lbl.pack(side="left")
        self.threshold.configure(
            command=lambda v: self.thresh_lbl.configure(text=f"{v:.1f}"))
        ctk.CTkLabel(pf, text="  Строк:").pack(side="left", padx=10)
        self.n_rows = ctk.CTkEntry(
            pf, width=90,
            placeholder_text=str(cfg.get("scan", {}).get("default_rows", 10000)))
        self.n_rows.pack(side="left", padx=5)
        self.use_api_var = ctk.BooleanVar(value=True)
        ctk.CTkCheckBox(pf, text="Через API",
                        variable=self.use_api_var).pack(side="left", padx=15)

        self.scan_progress = ctk.CTkProgressBar(frame)
        self.scan_progress.pack(fill="x", padx=20, pady=8)
        self.scan_progress.set(0)
        self.scan_status = ctk.CTkLabel(frame, text="Ожидание...", text_color="gray")
        self.scan_status.pack()

        ctk.CTkButton(frame, text="▶  Запустить сканирование", height=44,
                      font=ctk.CTkFont(size=14, weight="bold"),
                      command=self._run_scan).pack(pady=8)

        self.scan_result = ctk.CTkTextbox(
            frame, height=290, font=ctk.CTkFont(family="monospace", size=12))
        self.scan_result.pack(fill="both", expand=True, padx=20, pady=5)
        return frame
    ...
    def _print_results(self, log_ui, data: dict, mode: str):
        threat = data.get("threat", "—")
        color  = ("#e74c3c" if threat == "ВЫСОКАЯ"
                  else "#f39c12" if threat == "СРЕДНЯЯ" else "#2dc97e")
        log_ui(f"\n{'='*52}")
        log_ui(f"  РЕЗУЛЬТАТЫ [{mode.upper()}]")
        log_ui(f"{'='*52}")
        log_ui(f"  Всего:          {data.get('total_rows',0):,}")
        log_ui(f"  Нормальных:     {data.get('normal',0):,}")
        log_ui(f"  Аномалий:       {data.get('anomalies',0):,}  ({data.get('pct',0):.2f}%)")
        ...
        log_ui(f"{'='*52}")
        log_ui("  → Перейди на «Отчёт» для генерации PDF")
        self.scan_status.configure(text=f"Готово — угроза: {threat}", text_color=color)
...
"""
    ...
    def _page_about(self):
        ...
        for key, val in [
            ...
            ("Авторы",       "Аманглди Манас · Курманов Искандер · Куанышбек Бекарус"),
